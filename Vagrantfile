...
Vagrant.configure("2") do |config|
  ...
  config.trigger.after [:reload, :resume, :up] do |trigger|
    ...
    trigger.ruby do |env, machine|
      local_ips = `vagrant ssh #{machine.name} -c "ifconfig | sed -En 's/127.0.0.1//;s/.*inet (addr:)?(([0-9]*\\.){3}[0-9]*).*/\\2/p'" 2>/dev/null`
      local_ips.split("\r\n").each { |local_ip| puts "    #{machine.name}: http://#{local_ip}/smokeping" }
    end
  end
end
